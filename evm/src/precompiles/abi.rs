...
fn encode_output(
    ...
    if addr == Address::ZERO.to_string() || addr == "Invalid network" {
        return Err(PrecompileError::Other("Invalid vout address".to_string()));
    }
...
fn encode_input(input: &GetRawTransactionResultVin) -> Result<Input, PrecompileError> {
    let prev_tx_hash = input
        .txid
        .ok_or_else(|| PrecompileError::Other("Missing vin txid".to_string()))?;
    ...
    let output_index = input
        .vout
        .ok_or_else(|| PrecompileError::Other("Missing vout".to_string()))?;
