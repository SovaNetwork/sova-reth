...
mod tests {
    ...
    fn parse_known_chain_spec() {
        for &chain in SovaChainSpecParser::SUPPORTED_CHAINS {
            assert!(<SovaChainSpecParser as ChainSpecParser>::parse(chain).is_ok());
        }
    }
  ...
  "config": {
    ...
    "osakaTime": 0
        ...
        let spec = <SovaChainSpecParser as ChainSpecParser>::parse(s).unwrap();
