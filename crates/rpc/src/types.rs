...
use sova_primitives::tx::envelope::SovaTxEnvelope;


/// Sova Transaction type
#[derive(
...
pub struct Transaction {
    ...
    #[deref]
    #[deref_mut]
    pub inner: alloy_rpc_types_eth::Transaction<SovaTxEnvelope>,
...
impl AsRef<SovaTxEnvelope> for Transaction {
    fn as_ref(&self) -> &SovaTxEnvelope {
        self.inner.as_ref()
    }
}
...
mod tx_serde {
    ...
    /// field if inner [`SovaTxEnvelope`] did not consume it.
    ...
    impl From<Transaction> for TransactionSerdeHelper {
        fn from(value: Transaction) -> Self {
            ...
            let from = if matches!(inner.inner(), SovaTxEnvelope::L1Block(_)) {
                None
            } else {
                Some(inner.signer())
            };
    ...
    impl TryFrom<TransactionSerdeHelper> for Transaction {
        ...
        fn try_from(value: TransactionSerdeHelper) -> Result<Self, Self::Error> {
            ...
            let from = if let Some(from) = other.from {
                ...
                match &inner {
                    SovaTxEnvelope::L1Block(tx) => tx.from,
                    _ => {
...
mod tests {
    ...
    fn can_deserialize_deposit() {
        ...
        let SovaTxEnvelope::L1Block(inner) = tx.as_ref() else {
