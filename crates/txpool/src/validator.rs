...
#[derive(Debug, Clone)]
pub struct SovaTransactionValidator<Client, Tx> {
...
impl<Client, Tx> SovaTransactionValidator<Client, Tx> {
    ...
    Tx: EthPoolTransaction + MaybeInteropTransaction,
{
    ...
    /// Create a new [`SovaTransactionValidator`] with the given [`OpL1BlockInfo`].
    ...
    ///
    /// This behaves the same as [`SovaTransactionValidator::validate_one_with_state`], but creates
...
impl<Client, Tx> TransactionValidator for SovaTransactionValidator<Client, Tx>
where
    Client: ChainSpecProvider<ChainSpec: OpHardforks> + StateProviderFactory + BlockReaderIdExt,
    Tx: EthPoolTransaction + MaybeInteropTransaction,
{
