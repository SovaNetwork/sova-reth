...
use super::envelope::SovaTxEnvelope;
...
impl SovaPooledTransaction {
    ...
    /// Converts the transaction into the optimism [`SovaTxEnvelope`].
    pub fn into_op_envelope(self) -> SovaTxEnvelope {
        match self {
            Self::Legacy(tx) => tx.into(),
            Self::Eip2930(tx) => tx.into(),
            Self::Eip1559(tx) => tx.into(),
...
impl From<Signed<TxLegacy>> for SovaPooledTransaction {
    fn from(v: Signed<TxLegacy>) -> Self {
        Self::Legacy(v)
    }
}

impl From<Signed<TxEip2930>> for SovaPooledTransaction {
    fn from(v: Signed<TxEip2930>) -> Self {
        Self::Eip2930(v)
    }
}

impl From<Signed<TxEip1559>> for SovaPooledTransaction {
    fn from(v: Signed<TxEip1559>) -> Self {
        Self::Eip1559(v)
    }
}

impl From<Signed<TxEip4844>> for SovaPooledTransaction {
    fn from(v: Signed<TxEip4844>) -> Self {
        Self::Eip4844(v)
    }
}

impl From<Signed<TxEip7702>> for SovaPooledTransaction {
    fn from(v: Signed<TxEip7702>) -> Self {
        Self::Eip7702(v)
    }
}

impl Hash for SovaPooledTransaction {
    fn hash<H: Hasher>(&self, state: &mut H) {
        self.trie_hash().hash(state);
    }
}

impl Encodable for SovaPooledTransaction {
...
impl Decodable for SovaPooledTransaction {
    /// Decodes an enveloped [`SovaPooledTransaction`].
    ///
...
impl Encodable2718 for SovaPooledTransaction {
...
impl Decodable2718 for SovaPooledTransaction {
    fn typed_decode(ty: u8, buf: &mut &[u8]) -> Eip2718Result<Self> {
            ...
            .map_err(|_| alloy_rlp::Error::Custom("unexpected tx type"))?
        {
            ...
            SovaTxType::Eip7702 => Ok(TxEip7702::rlp_decode_signed(buf)?.into()),
            SovaTxType::Legacy => Err(Eip2718Error::UnexpectedType(SovaTxType::Legacy.into())),
            SovaTxType::L1Block => Err(Eip2718Error::UnexpectedType(SovaTxType::L1Block.into())),
        }
    }
...
impl Transaction for SovaPooledTransaction {
...
impl Typed2718 for SovaPooledTransaction {
    fn ty(&self) -> u8 {
        match self {
...
impl From<SovaPooledTransaction> for TxEnvelope {
    fn from(tx: SovaPooledTransaction) -> Self {
        tx.into_envelope()
    }
}

impl From<SovaPooledTransaction> for SovaTxEnvelope {
    fn from(tx: SovaPooledTransaction) -> Self {
        tx.into_op_envelope()
    }
}
...
mod tests {
    ...
    fn invalid_legacy_pooled_decoding_input_too_short() {
        ...
        for hex_data in &input_too_short {
            ...
            let res = SovaPooledTransaction::decode(input_rlp);
            ...
            let input_rlp = &mut &hex_data[..];
            let res = SovaPooledTransaction::decode_2718(input_rlp);
    ...
    fn legacy_valid_pooled_decoding() {
        ...
        let res = SovaPooledTransaction::decode_2718(&mut &data[..]);
        assert!(res.is_ok());
    }
}
