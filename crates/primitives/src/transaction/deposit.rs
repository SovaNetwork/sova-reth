...
impl SovaTxDeposit {
    /// Decodes the inner [SovaTxDeposit] fields from RLP bytes.
    ///
    ...
    /// Calculates a heuristic for the in-memory size of the [SovaTxDeposit] transaction.
    #[inline]
...
impl Typed2718 for SovaTxDeposit {
    fn ty(&self) -> u8 {
        SovaTxType::Deposit as u8
    }
}

impl Transaction for SovaTxDeposit {
...
impl Encodable2718 for SovaTxDeposit {
    fn type_flag(&self) -> Option<u8> {
        Some(SovaTxType::Deposit as u8)
    }
...
impl Decodable2718 for SovaTxDeposit {
    fn typed_decode(ty: u8, data: &mut &[u8]) -> Eip2718Result<Self> {
        ...
        if ty != SovaTxType::Deposit as u8 {
...
impl Encodable for SovaTxDeposit {
...
impl Decodable for SovaTxDeposit {
    fn decode(data: &mut &[u8]) -> alloy_rlp::Result<Self> {
        Self::rlp_decode(data)
    }
}

impl Sealable for SovaTxDeposit {
...
impl DepositTransaction for SovaTxDeposit {
    #[inline]
    fn source_hash(&self) -> Option<B256> {
...
///
/// This function can be used as `serialize_with` serde attribute for the [`SovaTxDeposit`] and will
/// flatten [`SovaTxDeposit::signature`] into response.
#[cfg(feature = "serde")]
pub fn serde_deposit_tx_rpc<T: serde::Serialize, S: serde::Serializer>(
    ...
    SerdeHelper { value, signature: SovaTxDeposit::signature() }.serialize(serializer)
}
...
mod tests {
    ...
    fn test_rlp_roundtrip() {
        ...
        let tx_a = SovaTxDeposit::decode(&mut bytes[1..].as_ref()).unwrap();
    ...
    fn test_encode_decode_fields() {
        ...
        let decoded = SovaTxDeposit::rlp_decode_fields(&mut &buffer[..]).expect("Failed to decode");
...
/// Bincode-compatible [`SovaTxDeposit`] serde implementation.
...
pub(super) mod serde_bincode_compat {
    ...
    /// Bincode-compatible [`super::SovaTxDeposit`] serde implementation.
    ///
    ...
    /// struct Data {
    ///     #[serde_as(as = "serde_bincode_compat::SovaTxDeposit")]
    ///     transaction: SovaTxDeposit,
    /// }
    /// ```
    #[derive(Debug, Serialize, Deserialize)]
    pub struct SovaTxDeposit<'a> {
        source_hash: B256,
        from: Address,
        #[serde(default)]
        to: TxKind,
    ...
    impl<'a> From<&'a super::SovaTxDeposit> for SovaTxDeposit<'a> {
        fn from(value: &'a super::SovaTxDeposit) -> Self {
            Self {
    ...
    impl<'a> From<SovaTxDeposit<'a>> for super::SovaTxDeposit {
        fn from(value: SovaTxDeposit<'a>) -> Self {
            Self {
                source_hash: value.source_hash,
                from: value.from,
                to: value.to,
    ...
    impl SerializeAs<super::SovaTxDeposit> for SovaTxDeposit<'_> {
        fn serialize_as<S>(source: &super::SovaTxDeposit, serializer: S) -> Result<S::Ok, S::Error>
        where
            S: Serializer,
        {
            SovaTxDeposit::from(source).serialize(serializer)
        }
    }

    impl<'de> DeserializeAs<'de, super::SovaTxDeposit> for SovaTxDeposit<'de> {
        fn deserialize_as<D>(deserializer: D) -> Result<super::SovaTxDeposit, D::Error>
        where
            D: Deserializer<'de>,
        {
            SovaTxDeposit::deserialize(deserializer).map(Into::into)
        }
    }
    ...
    mod tests {
        ...
        use super::super::{SovaTxDeposit, serde_bincode_compat};
        ...
        fn test_tx_deposit_bincode_roundtrip() {
            ...
            let data = Data {
                transaction: SovaTxDeposit::arbitrary(&mut arbitrary::Unstructured::new(&bytes))
                    .unwrap(),
            };
